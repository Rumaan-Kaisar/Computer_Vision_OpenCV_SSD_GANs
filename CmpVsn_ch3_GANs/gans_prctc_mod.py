...
class G(nn.Module): # We introduce a class to define the generator.
    ...
    def __init__(self): # __init__() will define the architecture of the generator.
...
class D(nn.Module): # the discriminator class

    def __init__(self):     # __init__() will define the architecture of the discriminator.
        super(D, self).__init__()   # We inherit from the nn.Module tools.

        # meta module of a NN containing a sequence of modules (convolutions, full connections, etc.).
        self.main = nn.Sequential(  
    ...
    def forward(self, input): # it will take an input that will be fed to the NN, and will return the output between 0 and 1.
...
criterion = nn.BCELoss() # the criterion object will measure the error between the prediction and the target.
...
for epoch in range(25): # We iterate over 25 epochs.
    for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset.
        ...
        # --------    1st Step: Updating the weights of the discriminator NN    --------

        netD.zero_grad() # We initialize to 0 the gradients of the discriminator with respect to the weights.
        
        # sub-step 1 : Training the discriminator with a real image of the dataset
        real, _ = data      # get a real image of the dataset which will be used to train the discriminator.
        input = Variable(real) # We wrap it in a variable, used it from 'torch.autograd'.
        target = Variable(torch.ones(input.size()[0])) # We get the target.
        output = netD(input)    # forward propagate 'real image' into the discriminator NN to get the prediction (a value between 0 and 1).
        errD_real = criterion(output, target)   # We compute the loss between the predictions (output) and the target (equal to 1).
        
        # sub-step 2 : Training the discriminator with a fake image generated by the generator
        noise = Variable(torch.randn(input.size()[0], 100, 1, 1))   # We make a random input vector (noise) of the generator.
        fake = netG(noise)  # We forward propagate random input vector into the GENERATOR NN to get some fake generated images.
        target = Variable(torch.zeros(input.size()[0]))     # We get the target.
        output = netD(fake.detach()) # forward propagate the fake generated images into the DISCRIMINATOR NN to get the prediction (a value between 0 and 1).
        errD_fake = criterion(output, target) # We compute the loss between the prediction (output) and the target (equal to 0).

        # sub-step 3 : Backpropagating the total error
        ...
        target = Variable(torch.ones(input.size()[0])) # We get the target.
        output = netD(fake) # forward propagate the generated images into discriminator NN to get the prediction (a value between 0 and 1).
        ...
        # --------    3rd Step: Printing losses, saving real & generated images of the minibatch every 100 steps    --------
...
#            real2 = real.sub_(0.5.div_(0.5)
            vutils.save_image(real, '%s/real_samples.png' % "./results") # We save the real images of the minibatch.
            fake = netG(noise) # We get our fake generated images.
            # We also save the fake generated images of the minibatch.
#            vutils.save_image(fake.data, '%s/fake_samples_epoch_%03d.png' % ("./results", epoch), normalize = True) 
#            fake2 = fake.sub_(0.5.div_(0.5)
